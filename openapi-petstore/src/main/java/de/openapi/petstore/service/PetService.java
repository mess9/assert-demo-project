...
public class PetService {
  ...
  public PetService(@Autowired PetRepository petRepository, @Value("${openapi.pet.enabled}") String enabled) {
    ...
    if (enabled.equals("mock") && !petRepository.findAll().isEmpty()) {
      ...
      pets.add(createPet(ObjectId.get(), categories.get(1), "Cat 1", new String[]{"cat1.jpg"},
          new String[]{"tag1", "tag2"}, StatusEnum.AVAILABLE));
      pets.add(createPet(ObjectId.get(), categories.get(1), "Cat 2", new String[]{"cat2.jpg"},
          new String[]{"tag2", "tag3"}, StatusEnum.AVAILABLE));
      pets.add(createPet(ObjectId.get(), categories.get(1), "Cat 3", new String[]{"cat3.jpg"},
          new String[]{"tag3", "tag4"}, StatusEnum.PENDING));

      pets.add(createPet(ObjectId.get(), categories.get(0), "Dog 1", new String[]{"dog1.jpg"},
          new String[]{"tag1", "tag2"},
          StatusEnum.AVAILABLE));
      pets.add(createPet(ObjectId.get(), categories.get(0), "Dog 2", new String[]{"dog2.jpg"},
          new String[]{"tag2", "tag3"}, StatusEnum.SOLD));
      pets.add(createPet(ObjectId.get(), categories.get(0), "Dog 3", new String[]{"dog3.jpg"},
          new String[]{"tag3", "tag4"}, StatusEnum.PENDING));

      pets.add(createPet(ObjectId.get(), categories.get(3), "Lion 1", new String[]{"lion1.jpg"},
          new String[]{"tag1", "tag2"}, StatusEnum.AVAILABLE));
      pets.add(createPet(ObjectId.get(), categories.get(3), "Lion 2", new String[]{"lion2.jpg"},
          new String[]{"tag2", "tag3"}, StatusEnum.AVAILABLE));
      pets.add(createPet(ObjectId.get(), categories.get(3), "Lion 3", new String[]{"lion3.jpg"},
          new String[]{"tag3", "tag4"}, StatusEnum.AVAILABLE));

      pets.add(createPet(ObjectId.get(), categories.get(2), "Rabbit 1", new String[]{"rabbit.jpg"},
          new String[]{"tag3", "tag4"}, StatusEnum.AVAILABLE));
      this.petRepository.saveAll(pets);
    }
  }

  private static Pet createPet(final ObjectId id, final Category cat, final String name,
      final String[] urls,
  ...
  public Pet getPetById(final ObjectId petId) {
    return petRepository.findById(petId);
  }

  public Pet updatePetWithForm(final ObjectId petId,
  ...
  public void deletePet(final ObjectId petId) {
    petRepository.deleteById(petId);
  }

  public Pet uploadFile(final ObjectId petId, final File file) {
    final var existingPet = petRepository.findById(petId);
    if (existingPet == null) {
      return null;
    }
