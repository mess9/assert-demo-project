...
public class _8AssertCatchExceptionTest {
    ...
    void getPetsById() {
        ...
        step("проверки", () -> SoftAssertions.assertSoftly(s -> {
            ...
            //2 - проверяем что выпало конкретное исключение
//            s.assertThatExceptionOfType(NullPointerException.class)
//                    .isThrownBy(() -> {
//                        s.assertThat(pet.getCategory()).satisfies(category -> {
//                            assertThat(category.getId())
//                                    .isEqualTo(petDb.getCategory().getId());
...
//            });
            ...
            //3 - ловим исключение трайкетчем и оборачиваем его в упавший ассерт
//            try {
//                s.assertThat(pet.getCategory()).satisfies(category -> {
//                    assertThat(category.getId())
//                            .isEqualTo(petDb.getCategory().getId());
